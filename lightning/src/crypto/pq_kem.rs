...
/// The ML-KEM-768 parameters (FIPS 203, Table 2) that shape a ciphertext: the modulus, the rank,
/// the polynomial degree and the compression widths of the two ciphertext components.
...
/// Derives a dummy ML-KEM-768 ciphertext from `seed` for padding a per-hop ciphertext list to its
...
pub(crate) fn dummy_ciphertext_from_seed(seed: &[u8; 32]) -> [u8; PQ_KEM_CT_LEN] {
	...
	// samples its public matrix. The 2048 draws of one buffer yield the 1024 coefficients needed
	// except with negligible probability, and the stream is simply extended if they do not.
...
/// Test-only: the number of `c1` coefficients of `ct` whose 10-bit compressed value has four
/// preimages under `Compress_10` rather than three. A real ML-KEM-768 ciphertext scores about 237 of
/// 768 and uniformly random bytes about 193, which is the counting test a hop could run on an entry
/// of a ciphertext list to tell padding from real ciphertexts.
#[cfg(test)]
pub(crate) fn four_preimage_count(ct: &[u8; PQ_KEM_CT_LEN]) -> usize {
	...
	for &byte in &ct[..MLKEM_C1_LEN] {
		...
		while nbits >= MLKEM_DU {
			if preimages[(acc & ((1 << MLKEM_DU) - 1)) as usize] == 4 {
				count += 1;
			}
			acc >>= MLKEM_DU;
			nbits -= MLKEM_DU;
...
mod tests {
	...
	fn dummy_ciphertexts_match_the_real_distribution() {
		...
		let n = 64;
		let real: usize =
			(0..n).map(|i| four_preimage_count(&encapsulate(&ek, &seed(i)).unwrap().1)).sum();
		let dummy: usize =
			(0..n).map(|i| four_preimage_count(&dummy_ciphertext_from_seed(&seed(i)))).sum();
		let random: usize = (0..n)
			.map(|i| {
				let mut bytes = [0u8; PQ_KEM_CT_LEN];
				ChaCha20::new(Key::new(seed(i)), Nonce::new([0; 12]), 0)
					.apply_keystream(&mut bytes);
				four_preimage_count(&bytes)
			})
			.sum();
		// Each entry scores about 237 when real or dummy and about 193 when random, with a standard
		// deviation near 13, so over 64 entries the sums sit near 15180 and 12340 with a standard
		// deviation near 100. The margins below are several standard deviations wide.
		assert!((real as i64 - dummy as i64).abs() < 1000, "real {} dummy {}", real, dummy);
		assert!(dummy > random + 1500, "dummy {} random {}", dummy, random);
